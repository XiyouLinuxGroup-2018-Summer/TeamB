...
void display_dir(int flag_param, char *path)
{
	...
	if(count > 256)
	{
		my_erro("too many files under this dir", __LINE__);
	}
	...
	//使用冒泡对文件名进行排序，排序后文件名按字母顺序存储于filenames
	for(i = 0; i < count - 1; i++)
	{
...
int main(int argc, char *argv[])
{
	...
	int flag_param = PARAM_NONE;//参数种类，既是否有-l，-a选项
	struct stat buf;
	/*命令行参数的解析，分析-l -al -a -la选项*/
	j = 0;
	num = 0;
	for(i = 1; i < argc; i++)
	{
	...
	do{
		...
		if(stat(path, &buf) == -1)
			my_erro("stat", __LINE__);
		if(S_ISDIR(buf.st_mode))      //argv[i]为目录
		{
			//如果目录的最后一个字符不是'/', 就加上'/'
