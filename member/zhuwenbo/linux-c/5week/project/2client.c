...
void *send_message(void *arg)
{
	...
	while(1)
	{
		...
		if(q == '4')
		{
			char *sql = "SELECT message from information";
			if( (mysql_real_query(con, sql, strlen(sql))) )
				my_err("select", __LINE__);
			res = mysql_store_result(con);
			while(row = mysql_fetch_row(res))
			{
				if(strncmp(row[0], "add_group", 9) == 0)
				{
					add_group(row[0], socket);
					char sql_delete[200];
					sprintf(sql_delete, "delete from information where message='%s'", row[0]);
					if( mysql_real_query(con, sql_delete, strlen(sql_delete)) )
						my_err("delete", __LINE__);
				}
				else if(strncmp(row[0], "add", 3) == 0)
				{
					add_friend(row[0], socket);
					char sql_delete[200];
					sprintf(sql_delete, "delete from information where message='%s'", row[0]);
					if( mysql_real_query(con, sql_delete, strlen(sql_delete)) )
						my_err("delete", __LINE__);
...
void *recv_message(void *arg)
{
	...
	{	
		...
		if(strncmp(readbuf, "add_group", 9) == 0)
		{
			printf("您有一条通知，请及时查看\n");
			char sql[200] = {0};
			sprintf(sql, "INSERT INTO information(message)values('%s')", readbuf);
			if( mysql_real_query(con, sql, strlen(sql)) )
