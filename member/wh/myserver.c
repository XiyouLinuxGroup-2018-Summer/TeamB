...
#define INVALID USERINFO		'n'		//用户信息无效
#define VALID_USERINFO			'y'		//用户信息有效

#define USERNAME				0		//用户名
#define PASSWORD					1		//密码
struct userinfo {						//定义用户名和密码的结构体
	char username[32];
	char password[32];
};
struct userinfo users[] = {
	{"linux","unix"},{"4507","4508"},{"clh","clh"},{"xl","xl"},{" "," "}
};										//以只含一个空格的字符串座位数组的结束标志

//查找用户名是否存在，存在则返回该用户名的下标，不存在返回-1,出错返回-2
...
void send_data(int conn_fd,const char *string)
{
	if(send(conn_fd,string,strlen(string),0) < 0) {
		my_err("send",__LINE__);		//__LINE__为此语句在源文件中的行数
	}
}
...
int main(void)
{
	...
	while(1) {
		//通过accept接受客户端的请求，并返回连接套接字用于收发数据
		...
		if((pid = fork()) == 0) {
			while(1) {
				...
				if(flag_recv == USERNAME) {			//接收到的是用户名
					...
					if(strcmp(users[name_num].password,recv_buf) == 0) {
						send_data(conn_fd,"y\n");
						send_data(conn_fd,"Welcome login my top serve\n");
			...
			close(conn_fd);							//关闭接受的连接请求，执行accept等待其他连接请求
