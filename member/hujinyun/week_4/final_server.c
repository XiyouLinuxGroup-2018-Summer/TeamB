...
struct connct_info      //conn_fd登陆结构体
{
    int fd;
    char name[10];
}connect_info[30];
...
void view_online_friend(struct message, int); //查看在线
...
struct users* read_input()//将文件中的信息读入到链表
{
    FILE *fp;
    struct users *p1,*p2;
...
void *client_t(void *arg)                                       //线程函数，专门用来处理客户端除过登录申请账户之外的请求
{
    ...
    while(1)
    {
        ...
        if(recv(connect_info[i].fd,recv_buf,sizeof(recv_buf),0) != BUFSIZE) //每次都是按BUFSIZE 长度来发送的,如果不是,那么就是发送失败
        {
            connect_info[i].fd = -1;
...
int main(int argc, char*argv[])
{
 //   int                         sock_fd,conn_fd;
    ...
    optval = 1;
    if(setsockopt(sock_fd,SOL_SOCKET,SO_REUSEADDR,(void*)&optval,sizeof(int))<0)//设定端口,要是宜昌退出还可以再绑定
    {
