...
#define W_NAPA 'n'                  //用户信息无效
#define R_NAPS 'y'                  //用户信息有效
#define BUFSIZE 1024
...
char myname[10];                    //存自己的名字
struct user friend[10];             //好友名字结构体数组
struct user member[10];             //群成员名字结构体数组
struct group group[3];              //群
...
void *sign_func()
{
    ...
    switch(chat.type)
    {
        case 'v'://查看所有好友
        {
            ...
            {
                fflush(stdout);
                if(strcmp(chat.name[i].username,chat.from))//除了自己以外的人输出
        ...
        case 't'://添加好友被同意
        {
            printf("\n[用户]%s 同意了您的好友请求,你们已经成为好友啦,快去找他聊聊天吧\n",chat.to);
            break;
        }
        case 'z'://查看在线好友
        {
...
void quit() //客户端退出函数,目的是为了让服务端释放套接字
{
    int ret;
...
void accept_add(int k)//接受添加
{
    ...
    for(t = k; k < 10; t++)
    {
        chat_t[t] = chat_t[t+1];//将我的消息提醒向前挪
    }
    b--;
}
void add_friend_req() //好友添加消息的通知
{
    int k,t,choice;
    system("clear");
    printf("-----------------------------------------\n");
...
void accept_add_group(int k) //加群同意与否
{
    int t;
    char choice;
    struct message chat;
...
int public_chat()                        //群聊
{
    ...
    system("clear");
    printf("-------你正在群 %s 里聊天，谨防诈骗！(如果想退出，输入bye)-------\n",near.to);
    while(1)
    {
        printf("%s",my_time());
        printf("%s说：",near.from);
        ...
        fclose(fp);
        if(strcmp(near.news,"bye") == 0)        //当输入quit,表示退出
        {
            return 0;
        }
        memcpy(near_buf,&near,LEN1);
