...
void accept_add_file();
void accept_add_file(int);
...
void accept_add_file(int k)
{
    int t;
    char choice;
    struct message chat;
    chat = chat_t[k];
    printf("[系统提示]用户 %s 想给你发送文件 < %s >,同意输入y,不同意输入n\n",chat.from,chat.file.file_name);
...
int chat_with()
{
    ...
    do
    {
        ...
        {  
            ...
            (!strcmp(choice,"2"))||(!strcmp(choice,"3")) || (!strcmp(choice,"4"))||(!strcmp(choice,"5"))||(!strcmp(choice,"6")))
                break;    
            else
             {
                 printf("\n无效输入，请重新输入\n");
...
void send_file()
{
    ...
    my_input(near.to,10);
    printf("请输入你想要发送的文件名或路径名:\n");
   // fflush(stdout);
   // setbuf(stdin,NULL);
    my_input(near.file.file_name,30);
    if((fp =fopen(near.file.file_name,"r") )== NULL )
    {
        my_err("fopen",__LINE__);
        exit(0);
    }
...
void *sign_func()
{
    while(1)
    {
        ...
        sleep(1);
        if((recv_t =my_recv(conn_fd,recv_buf,BUFSIZE,0)) <0)
        {
        ...
        switch(chat.type)
        {
            ...
            {
                printf("\n");
                printf("[系统提示] 用户 %s 想给您发送文件,请到 消息管理-文件发送请求 处理\n",chat.from);
                if(d<=29)
                    chat_t[d++] = chat;
                break;
            }
            case 'u'://开始发送文件通知
            {
                ...
                while(1)
                {
                    large = read(fd, chat.file.file_message, 500);
                    memcpy(near_buf, &chat, LEN1);
                    if(send(conn_fd,near_buf,BUFSIZE,0) != BUFSIZE)
                    {
                        my_err("send",__LINE__);
                    ...
                    if(sum == large)
                    {
                     ...
                     //   send(conn_fd,near_buf,BUFSIZE,0);
                        break;
                    }
            ...
            case '&':
            {
                printf("[开始接收文件]\n");
                int fd;
                if((fd = open("测试.txt",O_CREAT| O_RDWR | O_APPEND, 0644)) <0)
                    my_err("open",__LINE__);
                if(write(fd, chat.file.file_message, 500) <0)
                    {
...
int sign_in()           //登录函数
{
    ...
    while(1)
    {
        ...
        memset(apply_buf,0,sizeof(apply_buf));
        if((recv_t = my_recv(conn_fd,apply_buf,sizeof(apply_buf),0)) <0)
        {
            my_err("recv",__LINE__);
          //  exit(0);
        }
